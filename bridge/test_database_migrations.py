...
class MigrationTests(unittest.TestCase):
    ...
    def test_legacy_upgrade_preserves_records(self):
        initialize_database(self.path)
        self.execute("INSERT INTO dynasties VALUES ('id','Sample'); DROP TABLE recommendation_events; DROP TABLE recommendations; PRAGMA user_version=0;")
        initialize_database(self.path)
        connection = sqlite3.connect(self.path)
        try:
    ...
    def test_version_one_upgrade_preserves_full_observation(self):
        initialize_database(self.path)
        self.execute("DROP TABLE recommendation_events; DROP TABLE recommendations;"
    ...
    def test_version_two_validation_failure_rolls_back_new_tables(self):
        initialize_database(self.path)
        self.execute('DROP TABLE recommendation_events; DROP TABLE recommendations; PRAGMA user_version=1;')
        before = self.state()
